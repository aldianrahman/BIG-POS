    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                if (qty > maxStock) {
                    Text("Melebihi stok tersedia", color = MaterialTheme.colorScheme.error)
                }
                Text(
                    "Subtotal: ${Formatters.rupiah(unitPrice * (if (valid) qty else 0))}",
        ...
        confirmButton = {
            TextButton(onClick = { onConfirm(qty) }, enabled = valid) { Text("Tambah") }
        },
